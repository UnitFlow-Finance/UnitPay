...
/**
 * Flags an escrow as disputed, freezing it until the arbiter calls
 * release() or refund(). Callable by either payer or payee, requires an
 * arbiter to have been set at creation — enforced on-chain, not here.
 */
export async function POST(request: Request) {
  ...
  try {
    ...
    const response = await circleClient.createUserTransactionContractExecutionChallenge({
      ...
      abiFunctionSignature: "dispute(uint256)",
